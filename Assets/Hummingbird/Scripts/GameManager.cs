...
public class GameManager : MonoBehaviour
{
    ...
    private void EndGame()
    {
        ...
        if (player.NectorObtained >= opponent.NectorObtained )
        {
    ...
    private void Update()
    {
        if (State == GameState.Playing)
        {
            ...
            if (TimeRemaining <= 0f ||
                player.NectorObtained >= maxNectar ||
                opponent.NectorObtained >= maxNectar)
            {
                EndGame();
            }
            ...
            uiController.SetPlayerNectar(player.NectorObtained / maxNectar);
            uiController.SetOpponentNectar(opponent.NectorObtained / maxNectar);
        }
